...
export const getSSEInfoResponse = (port: number) => ({
  name: SERVER_INFO.displayName,
  version: SERVER_INFO.version,
  protocol: 'Model Context Protocol (MCP) with Server-Sent Events',
  message: 'This is an MCP server with SSE transport. Use an MCP client to connect.',
...
export const getHTTPInfoResponse = (port: number) => ({
  name: SERVER_INFO.displayName,
  version: SERVER_INFO.version,
  protocol: 'Model Context Protocol (MCP) with Streamable HTTP',
  message: 'This is an MCP server with Streamable HTTP transport. Use an MCP client to connect.',
  transport: 'Streamable HTTP',
  usage: {
    'MCP Inspector': 'npx @modelcontextprotocol/inspector',
    'Connection URL': `http://localhost:${port}/mcp`,
    Method: 'POST with JSON-RPC 2.0 format',
    'Connection Flow': 'Direct POST requests to /mcp endpoint',
  },
  capabilities: {
    tools: 0,
    resources: 0,
